...
export class ArticlesService {
	...
	async findRecent(): Promise<Article[]> {
		...
		for (let i = 0; i < 30; i++) {
			...
			data.push({
				...
				image: _dribbble('picture')
					.children('img')
					[i].attribs.src.replace(imgSizeRegex4, '_4x')
					.replace(imgSizeRegex2, '_2x'),
				link:
					'https://www.dribbble.com' +
					_dribbble('.dribbble-link')[i].attribs.href,
				website: 'DRIBBBLE',
		...
		for (let i = 0; i < 20; i++) {
			...
			data.push({
				//temporary id
				id: Math.floor(Math.random() * 100 ** 10),
				author: _awwwards('.content-view').find(_awwwards('.by')).find('a')[i]
					.children[0].data,
				title: _awwwards('.content-view').find(_awwwards('h3')).find('a')[i]
					.children[0].data,
				thumbnail: _awwwards('.content-view')
					.find(_awwwards('.rollover'))
					.find(_awwwards('.box-photo'))
					.find('img')
					[i].attribs['data-srcset'].match(getImgUrlRegex)[0],
				image: _awwwards('.content-view')
					.find(_awwwards('.rollover'))
					.find(_awwwards('.box-photo'))
					.find('img')
					[i].attribs['data-srcset'].match(getImgUrlRegex)[1],
				link:
					'https://www.awwwards.com' +
					_awwwards('.content-view')
						.find(_awwwards('.rollover'))
						.find('a')
						.not('.js-visit-item')[i].attribs.href,
				website: 'AWWWARDS',
				createdAt: new Date().toISOString(),
				//@todo
				stars: '1000',
		...
		for (let i = 0; i < 20; i++) {
			...
			data.push({
				...
				//@todo author
				author: 'in progress',
				title: _behance('.TitleOwner-limitHeight-2_Y').find('a[data-ut=title]')[
					i
				].children[0].data,
		...
		if (articles) {
			articles = await Promise.all(
				articles.map(async (article) => {
					...
					const checkArticle = await this.articleModel.findOne({
						title: article.title,
					});
					if (!checkArticle) {
	...
	async findAll(phrase: string): Promise<Article[]> {
		...
		return this.articleModel
			.find({
				$or: [
					{ title: { $regex: likeStatement } },
					{ website: { $regex: likeStatement } },
					{ author: { $regex: likeStatement } },
				],
			})
			.exec();
	}
